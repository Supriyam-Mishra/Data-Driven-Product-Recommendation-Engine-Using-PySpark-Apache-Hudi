...
def main(config_path: str):
    ...
    input_path = cfg["company_sales"]["input_path"]
    hudi_output = cfg["company_sales"]["hudi_output_path"]
    quarantine_path = hudi_output + "/quarantine/company_sales"
    ...
    # Remove duplicates on item_id
    df = df.dropDuplicates(["item_id", "sale_date"])  # per day; change if needed

    # Fill missing numeric with 0
    df = df.fillna({"units_sold": 0, "revenue": 0.0})

    df = add_dq_flags_company(df)
